...
struct LoginScreenView: View {
    ...
    func loginUser(email: String, firebaseToken: String, completion: @escaping (Bool, String) -> Void) {
        guard let url = URL(string: "http://13.218.5.40:8080/login") else {
    ...
    func handleLogin() {
        ...
        Auth.auth().signIn(withEmail: loginEmail, password: loginPassword) { result, error in
            ...
            if let error = error {
                ...
                print("Firebase login successful for UID: \(user.uid)")
                user.getIDToken { idToken, error in
                    if let error = error {
                        print("Failed to get Firebase ID Token: \(error.localizedDescription)")
                    } else if let idToken = idToken {
                        print("Firebase ID Token (first 50 characters): \(idToken.prefix(50))")
                        loginUser(email: loginEmail, firebaseToken: idToken) { success, message in
                            DispatchQueue.main.async {
                                print(success ? "Backend login success: \(message)" : "Backend login failed: \(message)")
                            }
                // END OF backend implmentation
...
struct SignUpView: View {
    ...
    // BACKEND IMPLEMENTAION FOR THE FRONTEND
    ...
    // END OF THE BACKEND IMPLEMENTAION FOR THE FRONTEND
    ...
    func saveUserData(uid: String) {
        ...
        userDoc.setData(userData) { error in
            if let error = error {
                ...
                // BACKEND IMPLEMENTAION in Frontend:  Now we can now register user on backend using Elastic IP
                registerUser(email: userInfo.email) { success, message in
                    ...
                    // End of BACKEND IMPLEMENTAION in Frontend
                }
            }
        }
    }
